...
bool WriteToFile(const char *filename) {
    ...
    int allsize = g_CharList.size() * 6 * 4;
    for (int i = 0; i < g_CharList.size(); i++) {
        allsize += g_CharList[i].bitmap_height * g_CharList[i].bitmap_width;
    }
    ...
    for (int i = 0; i < g_CharList.size(); i++) {
        ...
        int bitmap_size = fileinfo.bitmap_width * fileinfo.bitmap_height;
