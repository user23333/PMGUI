...
bool mywcsicmp(LPCWSTR str1, LPCWSTR str2) {
...
void myzeromem(LPVOID addr, SIZE_T sz) {
    while (sz)
        ((PUCHAR)addr)[sz-- - 1] = 0;
}
void mymemcpy(PVOID dst, LPCVOID src, SIZE_T sz) {
    while (sz)
        ((PUCHAR)dst)[sz - 1] = ((PUCHAR)src)[sz-- - 1];
}
void mystrcpy(LPSTR dst, LPCSTR src) {
    while (*dst++ = *src++);
}
