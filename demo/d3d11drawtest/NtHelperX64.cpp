...
LONG GetFunctionSSDTIndex(LPCSTR FunName) {
	if (g_ntdll == 0) {
		g_ntdll = GetMemModuleHandleX64(XorStr("ntdll.dll"));
	}
	PUCHAR Addr = (PUCHAR)GetMemProcAddressX64(g_ntdll, FunName);
	if (Addr == 0)return -1;
...
LONG GetWin32uSSSDTIndex(LPCSTR FunName) {
	if (g_win32u == 0) {
		g_win32u = GetMemModuleHandleX64(XorStr("win32u.dll"));
	}
	PUCHAR Addr = (PUCHAR)GetMemProcAddressX64(g_win32u, FunName);
	if (Addr == 0)return -1;
	for (int i = 0; i < 0x100; i++) {
		if (Addr[i] == 0xB8) {
			return *(LONG*)(&Addr[i + 1]);
...
NTROUTINE MakeSSDTFunction(ULONG SSDTIndex) {
	...
	PVOID pCode = ((PUCHAR)g_SyscallBuffer) + SSDTIndex * 0x27;

	//g_LastIndex++;
	return (NTROUTINE)pCode;
}
ULONG GetNtUserGetAsyncKeyStateIndex() {
	ULONG64 hmod = GetMemModuleHandleX64(XorStr("win32u.dll"));
	if (hmod) {
		PUCHAR Addr = (PUCHAR)GetMemProcAddressX64(hmod, XorStr("NtUserGetAsyncKeyState"));
		if (Addr) {
		...
		hmod = GetMemModuleHandleX64(XorStr("user32.dll"));
		if (hmod) {
			PUCHAR Addr = (PUCHAR)GetMemProcAddressX64(hmod, XorStr("GetAsyncKeyState"));
			if (Addr) {
...
void AnsiToUnicode(LPCSTR AnsiStr, LPWSTR UnicodeStrBuffer, ULONG MaxLenth) {
	MultiByteToWideChar(0, 0, AnsiStr, -1, UnicodeStrBuffer, MaxLenth);
	return;
}
void UnicodeToAnsi(LPCWSTR UnicodeStr, LPSTR AnsiStrBuffer, ULONG MaxLenth) {
	WideCharToMultiByte(0, 0, UnicodeStr, -1, AnsiStrBuffer, MaxLenth, NULL, NULL);
	return;
}
...
ULONG myGetNtBuildNumber() {
	if (g_BuildNumber == 0) {
		...
		ULONG64 ntdll = GetMemModuleHandleX64(XorStr("ntdll.dll"));
		typedef MYNTSTATUS(*fnRtlGetVersion)(MYRTL_OSVERSIONINFOW *info);
		fnRtlGetVersion fn = (fnRtlGetVersion)GetMemProcAddressX64(ntdll, XorStr("RtlGetVersion"));
